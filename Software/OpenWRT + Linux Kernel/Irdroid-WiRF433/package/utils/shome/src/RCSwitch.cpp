...
//  this->nReceiverInterrupt = -1;
  this->nTransmitterPin = 0;
//  RCSwitch::nReceivedValue = NULL;
//  this->setPulseLength(timer);
//  this->setRepeatTransmit(40);
...
f1 = fopen ("/sys/class/gpio/gpio0/value", "w");
fprintf (f1, "%d", state);
fclose (f1);
}
...
f1 = fopen ("/sys/class/gpio/gpio0/direction", "w");
fprintf (f1, "out");
fclose (f1);
...
void RCSwitch::transmit(int nHighPulses, int nLowPulses) {
    ...
    if (this->nTransmitterPin != -1) {
        ...
        digitalWrite(this->nTransmitterPin, true);
        delayMicroseconds( this->nPulseLength * nHighPulses);
        digitalWrite(this->nTransmitterPin, false);
